...
module.exports = async (req, res) => {
  ...
  try {
    ...
    snap.forEach(doc => {
      ...
      let value;
      try {
        if (data._json) value = JSON.parse(data.v);
        else if (data.v !== undefined) value = data.v;
        else if (data.value !== undefined) value = JSON.parse(data.value); // legacy fallback
        else return;
      } catch { return; /* skip malformed */ }

      if (key === 'prog')         prog         = value;
      if (key === 'eventName')    eventName    = value;
      if (key === 'eventDate')    eventDate    = value;
      if (key === 'programStart') programStart = value;
